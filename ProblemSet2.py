...
def problem2_3(ne):
    for element in ne:
        print(element, "has", len(element), "letters.")
...
def problem2_4():
    ...
    listofrandom=[]
    for i in range(10):
        x=30+5*random.random()
        listofrandom.append(x)
    print(listofrandom)
...
def problem2_7():
    ...
    sideone=input("Enter length of side one: ")
    sidetwo=input("Enter length of side two: ")
    sidethree=input("Enter length of side three: ")
    sone=float(sideone)
    stwo=float(sidetwo)
    sthree=float(sidethree)
    s=0.5*(sone+stwo+sthree)
    result=s*(s-sone)*(s-stwo)*(s-sthree)
    print("Area of a triangle with sides", sone, stwo, sthree, "is", result**0.5)
